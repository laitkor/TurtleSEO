...
function getBlogs2() {
  var query = new google.gdata.blogger.BlogQuery(BLOGGER_LIST_URL); 
  BlogPress.service.getBlogFeed(query, handleBlogsFeed2, handleError);  
};
...
function handleBlogsFeed(resultsFeedRoot) {
  ...
  var str ='<h5>Select the Blog to be added</h5>';
  for (var i = 0; i < BlogPress.blogs.length; i++) {
    var blog = BlogPress.blogs[i];
    var blogTitle = blog.getTitle().getText();	
	str = str + '<input  type="checkbox" name="data[blogname][]" value="'+blogTitle+'" checked>&nbsp;'+blogTitle+"<br/>";	   
  }
  if(str!=""){
	  str = str + '&nbsp;&nbsp;<input type="submit" name="subm" value="Save" />';
  }
  changeStatus(str);
};

function handleBlogsFeed2(resultsFeedRoot) {
  var blogsFeed = resultsFeedRoot.feed;
  BlogPress.blogs = blogsFeed.getEntries();
  var str ='<h5>Select the Blog</h5>';
  	
  for (var i = 0; i < BlogPress.blogs.length; i++) {
    var blog = BlogPress.blogs[i];
    var blogTitle = blog.getTitle().getText();
	str = str + '<input  type="radio" name="blogname[]" onChange="updateCurrentBlogNum()" id="blogid'+i+'" value="'+i+'"';
	if(i==0)
		str = str + ' checked';
	str = str + '>&nbsp;'+blogTitle+"<br/>";	
   ...
   str = str + '&nbsp;&nbsp;<input type="button" onClick="saveAll()" name="subm" value="Save" />';
   changeStatus(str);
};
