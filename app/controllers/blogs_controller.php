...
class BlogsController extends AppController {
  ...
  /**
		This function is to use Google API.
  */
  function gdata($act="login",$article_id=NULL){	  
	  $this->set('close',"0");
	  ...
	  $user_post_limit=$plan[0]['UserPlan']['post_limit'];						
	  $this->layout = 'popup';
	  switch($act){
		case "add":
			$this->set('userid', $this->Session->read('user_id'));
			if (!empty($this->data)) 
			{
				foreach($this->data['blogname'] as $blogname){
					$count =$this->WpServer->query("SELECT COUNT(*) as coldata FROM wp_servers WHERE user_id=".$this->Session->read('user_id')." AND name LIKE '".$blogname." (Blogspot)"."' AND blogspot=1"); 					
					$sql ="INSERT INTO wp_servers SET 
							user_id=".$this->Session->read('user_id').",
							name='".$blogname." (Blogspot)"."',
							wp_admin_id='Google',
							wp_admin_password='*****',
							rpc_url='http://blogspots.com',
							active=1,
							blogspot=1";
					if(!$count[0][0]['coldata']){		
						$saved=$this->WpServer->query($sql);							
  ...
  */
	...
	{	
		if ($id <> "" )
		{
  			if( $this->WpServer->find( 'count', array('conditions' => array('WpServer.id' => $id, 'WpServer.user_id' => $this->Session->read('user_id'))) ) <= 0 )
			{
		...
		if (!empty($this->data)) 
		{
		  $raw_url = $this->data['WpServer']['name'];
		  $url = str_replace('http://','',$raw_url);
		  $this->data['WpServer']['name'] = "http://".$url;
		  
		  $raw_url=$this->data['WpServer']['rpc_url'];
		  $url=str_replace('http://','',$raw_url);
		  $this->data['WpServer']['rpc_url']="http://".$url;  
			//$this->data['WpServer']['rpc_url']=
	...
	*/
	function edit($id) 
	{
		...
		else
		{
		  ...
		  $url=str_replace('http://','',$raw_url);
		  $this->data['WpServer']['rpc_url']="http://".$url;  
		//manish start validation for limit
		 $users_limit= $this->Session->read('users_limit_crossed'); 
		 if(!isset($users_limit) && $users_limit!='true'){
			if ($this->WpServer->save($this->data))
			{		
	...
	function delete($id)
	{
		...
		if ($plan_data['UserPlan']['blog_to_del'] > 0 )
		{
			$d['UserPlan']['blog_to_del']=$plan_data['UserPlan']['blog_to_del'] - 1;
			$d['UserPlan']['blog_limit'] = $plan_data['UserPlan']['blog_limit'] + 1;
			$this->UserPlan->save($d);
		}	
		$this->WpServer->delete($id,true);
		$this->WpServer->query("delete from posted_articles where wp_server_id=$id");	
		$this->Session->setFlash('Blog has been deleted.');
	...
	function add_page($id=null)
	{
		...
		if( $id != null )
			...
			if(!empty($this->data))
			{
				...
				{
					$this->set('msg',"<font color='red'>Please fill all mandatory fields</font>");
				}
				else
				{	
					...
					try
					{
						...
						if(!$page_exists)
						{
							...
							$message="Page added to the server";
							$this->Session->setFlash($message);
						}
						else
						{
		...
		if(!empty($this->data))
		{
			...
			else
			{
				...
				if($this->client->query('wp.deletePage','',$server['WpServer']['wp_admin_id'],$server['WpServer']['wp_admin_password'],$page_id))
				{
					...
					$plan_data=$this->UserPlan->findByUserId($user_id);
					$this->UserPlan->id = $plan_data['UserPlan']['id'];
					if ($plan_data['UserPlan']['page_to_del'] > 0 )
					{
						$d['UserPlan']['page_to_del']=$plan_data['UserPlan']['page_to_del'] - 1;
						$d['UserPlan']['page_limit'] = $plan_data['UserPlan']['page_limit'] + 1;
						$this->UserPlan->save($d);
					}	
